...
describe('Band and Musician Models', () => {
    ...
    test("can add multiple songs to multiple bands", async function() {
        ...
        expect(song[0].year).toBe(2023)
        expect(song[1].year).toBe(2023)
    })

    test("can add many musicians to one band", async function() {
        const band = await Band.findByPk(1)
        await band.addMusicians([1,2])
        const musician = await band.getMusicians([1,2])
        // expect(musicians[0]).toEqual({id: 1, name: "Steve", instrument: "Drums", bandId: 1})
        expect(musician[0].name).toBe("Steve")
        expect(musician[1].name).toBe("Bob")
        expect(musician[0].instrument).toBe("Drums")
        expect(musician[1].instrument).toBe("Piano")
        expect(musician[0].bandId).toBe(1)
        expect(musician[1].bandId).toBe(1)
    })
